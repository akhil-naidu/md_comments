...
const AttributePool = require('md_mudoc-lite/static/js/AttributePool');
...
const padMessageHandler = require('md_mudoc-lite/node/handler/PadMessageHandler');
const readOnlyManager = require('md_mudoc-lite/node/db/ReadOnlyManager.js');
...
exports.eejsBlock_dd_insert = (hookName, args, cb) => {
  args.content += eejs.require('md_comments/templates/menuButtons.ejs');
  return cb();
};

exports.eejsBlock_mySettings = (hookName, args, cb) => {
  args.content += eejs.require('md_comments/templates/settings.ejs');
  return cb();
};
...
exports.padInitToolbar = (hookName, args, cb) => {
  ...
  const button = toolbar.button({
    command: 'addComment',
    localizationId: 'md_comments.add_comment.title',
...
exports.eejsBlock_editbarMenuLeft = (hookName, args, cb) => {
  ...
  args.content += eejs.require('md_comments/templates/commentBarButtons.ejs');
  return cb();
};

exports.eejsBlock_scripts = (hookName, args, cb) => {
  args.content += eejs.require('md_comments/templates/comments.html');
  args.content += eejs.require('md_comments/templates/commentIcons.html');
  return cb();
};

exports.eejsBlock_styles = (hookName, args, cb) => {
  args.content += eejs.require('md_comments/templates/styles.html');
  return cb();
};

exports.clientVars = (hook, context, cb) => {
  const displayCommentAsIcon =
    settings.md_comments ? settings.md_comments.displayCommentAsIcon : false;
  const highlightSelectedText =
    settings.md_comments ? settings.md_comments.highlightSelectedText : false;
  return cb({
