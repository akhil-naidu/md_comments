...
const browser = require('md_mudoc-lite/static/js/browser');
const commentBoxes = require('md_comments/static/js/commentBoxes');
const commentIcons = require('md_comments/static/js/commentIcons');
const commentL10n = require('md_comments/static/js/commentL10n');
const events = require('md_comments/static/js/copyPasteEvents');
const moment = require('md_comments/static/js/moment-with-locales.min');
const newComment = require('md_comments/static/js/newComment');
const padcookie = require('md_mudoc-lite/static/js/pad_cookie').padcookie;
const preCommentMark = require('md_comments/static/js/preCommentMark');
...
const Security = require('md_mudoc-lite/static/js/security');

const cssFiles = [
  'md_comments/static/css/comment.css',
  'md_comments/static/css/commentIcon.css',
];
...
MdComments.prototype.init = async function () {
  ...
  this.container.parent().on('click', '.comment-delete', async function () {
    ...
    try {
      ...
      $.gritter.add({
        title: html10n.translations['md_comments.error'] || 'Error',
        text: html10n.translations['md_comments.error.delete_unauth'] ||
  ...
  this.container.parent().on('click', '.comment-edit-submit', async function (e) {
    ...
    try {
      ...
      $.gritter.add({
        title: html10n.translations['md_comments.error'] || 'Error',
        text: html10n.translations['md_comments.error.edit_unauth'] ||
...
MdComments.prototype.displayNewCommentForm = function () {
  ...
  if (noTextSelected) {
    $.gritter.add({
      text: html10n.translations['md_comments.add_comment.hint'] ||
...
MdComments.prototype.saveComment = async function (data, rep) {
  ...
  this.ace.callWithAce((ace) => {
    ...
    // details at https://github.com/akhil-naidu/md_comments/issues/133
    rep = ace.ace_getRep();
...
const hooks = {
  ...
  postAceInit: (hookName, context, cb) => {
    ...
    if (!$('#editorcontainerbox').hasClass('flex-layout')) {
      $.gritter.add({
        title: 'Error',
        text: 'md_comments: Please upgrade to MuDoc 0.0.2 ' +
  ...
  postToolbarInit: (hookName, args, cb) => {
    ...
    editbar.registerCommand('addComment', () => {
      pad.plugins.md_comments.displayNewCommentForm();
    });
    return cb();
  },
  ...
  aceEditEvent: (hookName, context) => {
    ...
    if (eventType === 'unmarkPreSelectedTextToComment') {
      pad.plugins.md_comments.preCommentMarker.handleUnmarkText(context);
    } else if (eventType === 'markPreSelectedTextToComment') {
      pad.plugins.md_comments.preCommentMarker.handleMarkText(context);
    }

    if (['setup', 'setBaseText', 'importText'].includes(eventType)) return;

    if (context.callstack.docTextChanged && pad.plugins.md_comments) {
      pad.plugins.md_comments.setYofComments();
    }

    // some times on init md_comments is not yet on the plugin list
    if (pad.plugins.md_comments) {
      const commentWasPasted = pad.plugins.md_comments.shouldCollectComment;
      ...
      if (commentWasPasted && domClean) {
        pad.plugins.md_comments.collectComments(() => {
          pad.plugins.md_comments.collectCommentReplies();
          pad.plugins.md_comments.shouldCollectComment = false;
        });
      }
    }
    return;
